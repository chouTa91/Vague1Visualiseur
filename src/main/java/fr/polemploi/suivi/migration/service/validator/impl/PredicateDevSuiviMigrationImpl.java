...
/**
 * Implémentation de {@link PredicateSuiviMigration} pour la version de
 * l'<b>outillage 22M04</b>
 *
 * @author jbourrea
 *
 */
@Component
@Profile("22M04")
public class PredicateDevSuiviMigrationImpl implements PredicateSuiviMigration {
	...
	public Tuple<String, Predicate<Alert>> dl1CheckVolumetrieEcart(Alert alert) {
		...
		// SI écart entre les valeurs il y a ET que cette écart est supérieur à une
		// valeur dite "acceptable".
		Predicate<Alert> pr = input -> (!((Dl1Volumes) input).getVolumeLog()
				.equals(((Dl1Volumes) input).getVolumeOracle())
				&& ((Dl1Volumes) input).getVolumeLog() - ((Dl1Volumes) input).getVolumeOracle() < 10);
